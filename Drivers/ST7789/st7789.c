...
void ST7789_Init(void)
{
  ...
  ST7789_WriteCommand(ST7789_PORCTRL); // Set Porch Control
  {
  ...
  {
    uint8_t data[] = {0x00, 0x28, 0x00, 0x00, 0x00, 0x28};
...
void ST7789_DrawBitmap(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t *buff)
{
  ...
  // split data in small chunks because HAL can't send more than 64K at once
  ConvHL(buff, (int32_t)buff_size*2);
  ...
  while (buff_size > 0) {
    uint16_t chunk_size = buff_size > 65535 ? 65535 : buff_size;
      ...
      if (DMA_MIN_SIZE <= buff_size)
      {
        HAL_SPI_Transmit_DMA(&ST7789_SPI_PORT, buff, chunk_size*2);
        while (ST7789_SPI_PORT.hdmatx->State != HAL_DMA_STATE_READY)
        {}
      }
      else
        HAL_SPI_Transmit(&ST7789_SPI_PORT, buff, chunk_size*2, HAL_MAX_DELAY);
#else
      HAL_SPI_Transmit(&ST7789_SPI_PORT, (uint8_t*)buff, buff_size *2, HAL_MAX_DELAY);
#endif
    buff += chunk_size;
    buff_size -= chunk_size;
  }
